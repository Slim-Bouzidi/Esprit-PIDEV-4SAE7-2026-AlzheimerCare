...
export class DoctorReportsComponent implements OnInit, OnDestroy {
    ...
    private fichesPoll$?: Subscription;
    ...
    ngOnInit(): void {
        ...
        // Force reload after a short delay to ensure data is loaded
        setTimeout(() => {
            this.loadFichesRecues();
        }, 1000);
        ...
        // Fallback polling for fiches reçues (prevents manual page refresh)
        // This keeps "real-time" UX even when WS payload is missing/incomplete.
        this.fichesPoll$ = interval(3000).subscribe(() => this.loadFichesRecues());
        ...
        this.notifWs$ = this.doctorWs.notifications$.subscribe((msg: DoctorNotificationMessage) => {
            ...
            this.ngZone.run(() => {
                ...
                // Auto-update sections when notification arrives (no page reload)
                const isFicheNotif =
                    msg.referenceType === 'FICHE_TRANSMISSION' ||
                    msg.type === 'FICHE_ENVOYEE';

                if (isFicheNotif) {
                    const ficheId = msg.referenceId;
                    if (typeof ficheId === 'number') {
                        console.log('📄 Mise à jour temps réel de la fiche reçue:', ficheId);
                        this.addOrRefreshFicheRecue(ficheId);
                    }

                    // Robust fallback: even if payload lacks referenceId or DB is not updated yet,
                    // refresh the list shortly after the notification (still no page reload).
                    setTimeout(() => this.loadFichesRecues(), 500);
                }
                if (msg.referenceType === 'RAPPORT_HEBDOMADAIRE') {
                    console.log('📊 Rechargement des rapports hebdo...');
                    this.loadRapportsHebdo(); // Reload weekly reports
                }
            });
        });
    }
...
<div class="page">
  ...
  <div class="doc-footer">
    ...
    loadFichesRecues(): void {
        this.ficheApi.getAll().subscribe({
            next: (data: any[]) => {
                ...
                console.log('📥 Nombre total de fiches:', data.length);
                ...
                this.ngZone.run(() => {
                    ...
                    console.log('✅ Fiches filtrées (status=envoye):', this.fichesRecues.length, 'fiches');
                    console.log('✅ IDs des fiches filtrées:', this.fichesRecues.map(f => f.id));
                    this.fichesPagination.totalItems = this.fichesRecues.length;
                    this.fichesPagination.currentPage = 1;
                    this.applyFichesPagination();
                    console.log('📄 Fiches paginées affichées:', this.pagedFiches.length, 'fiches');
                    console.log('📄 IDs des fiches paginées:', this.pagedFiches.map(f => f.id));
                    console.log('📄 Contenu pagedFiches:', this.pagedFiches);
    ...
    /**
     * Real-time incremental update for "Fiches de transmission reçues".
     * Fetches only the changed fiche then inserts/updates it locally.
     * Falls back to reloading the whole list if needed.
     */
    private addOrRefreshFicheRecue(ficheId: number): void {
        this.ficheApi.getById(ficheId).subscribe({
            next: (fiche: any) => {
                // Only display sent fiches in "reçues"
                if (!fiche || fiche.statut !== 'envoye') {
                    // If the backend hasn't flipped statut yet, do a short delayed fallback refresh.
                    setTimeout(() => this.loadFichesRecues(), 700);
                    return;
                }

                const existingIdx = this.fichesRecues.findIndex(f => f?.id === ficheId);
                if (existingIdx >= 0) {
                    this.fichesRecues[existingIdx] = fiche;
                } else {
                    this.fichesRecues = [fiche, ...this.fichesRecues];
                }

                // Ensure newest first (dateCreation can be null depending on backend serialization)
                this.fichesRecues = [...this.fichesRecues].sort((a: any, b: any) =>
            ...
            error: (err) => {
                console.error('❌ Erreur chargement fiche reçue:', err);
                this.loadFichesRecues();
            }
        });
    }
    ...
    applyFichesPagination(): void {
        ...
        console.log(`🔢 Pagination: page=${this.fichesPagination.currentPage}, pageSize=${this.fichesPagination.pageSize}, startIdx=${startIdx}, endIdx=${endIdx}`);
        console.log(`🔢 fichesRecues.length=${this.fichesRecues.length}`);
        // Force change detection by creating new array
        this.pagedFiches = [...this.fichesRecues.slice(startIdx, endIdx)];
        console.log(`🔢 pagedFiches après slice:`, this.pagedFiches.length, 'fiches', this.pagedFiches.map(f => f.id));
    }
     ...
     */
    trackByFicheId(index: number, fiche: any): number {
        return fiche.id || index;
    }
}
