...
export class MedicalAgendaComponent implements OnInit {
  ...
  isFormulaireValide(): boolean {
    ...
    v.required('patientId', this.formulaire.patientId, 'Veuillez sélectionner un patient.');
    v.required('type', this.formulaire.type, 'Le type de rendez-vous est requis.');
    v.required('date', this.formulaire.date, 'La date est requise.');
    v.dateValid('date', this.formulaire.date, 'Date invalide.');
    v.required('heure', this.formulaire.heure, 'L\'heure est requise.');
    v.required('motif', this.formulaire.motif, 'Le motif est requis.');
    v.maxLength('motif', this.formulaire.motif, 300, 'Le motif ne doit pas dépasser 300 caractères.');
    v.required('lieu', this.formulaire.lieu, 'Le lieu est requis.');
    v.maxLength('lieu', this.formulaire.lieu, 200, 'Le lieu ne doit pas dépasser 200 caractères.');
    v.custom('duree', this.formulaire.duree <= 0, 'La durée doit être supérieure à 0.');
    v.maxLength('notesMedicales', this.formulaire.notesMedicales || '', 2000, 'Les notes ne doivent pas dépasser 2000 caractères.');
