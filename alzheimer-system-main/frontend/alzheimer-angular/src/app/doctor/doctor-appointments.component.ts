...
export class DoctorAppointmentsComponent implements OnInit {
    ...
    loadMockData(): void {
        // Charger les patients depuis la table patient de assistancequotidiennedb
    ...
    saveRendezVous(): void {
        ...
        v.required('patient', this.currentRendezVous.patient, 'Veuillez sélectionner un patient.');
        v.required('dateRendez', this.dateRendez, 'La date est requise.');
        v.required('heureRendez', this.heureRendez, 'L\'heure est requise.');
        v.required('type', this.currentRendezVous.type, 'Le type de rendez-vous est requis.');
        v.dateValid('dateRendez', this.dateRendez, 'Date invalide.');
        v.maxLength('motif', this.currentRendezVous.motif || '', 300, 'Le motif ne doit pas dépasser 300 caractères.');
        v.maxLength('notes', this.currentRendezVous.notes || '', 1000, 'Les notes ne doivent pas dépasser 1000 caractères.');
        v.maxLength('lieu', this.currentRendezVous.lieu || '', 200, 'Le lieu ne doit pas dépasser 200 caractères.');
        ...
        // Retrouver l'objet patient complet pour l'affichage du nom
        const patientId = Number(this.currentRendezVous.patient?.id || this.currentRendezVous.patient);
        const fullPatient = this.patients.find(p => p.id === patientId);
        ...
        if (this.isEditing && this.currentRendezVous.id) {
            ...
            this.rendezVousService.update(this.currentRendezVous.id, rdvToSend).subscribe({
                next: (updated) => {
                    // Toujours attacher le patient complet pour l'affichage du nom
                    if (fullPatient) {
                        updated.patient = fullPatient;
                    }
                ...
                error: (err) => {
                    ...
                    // Mise à jour locale en fallback pour que le tableau reflète le changement
                    if (fullPatient) {
                        const index = this.rendezVous.findIndex(r => r.id === this.currentRendezVous.id);
                        if (index !== -1) {
            ...
            this.rendezVousService.create(rdvToSend).subscribe({
                ...
                error: (err) => {
                    ...
                    // Création locale en fallback
                    if (fullPatient) {
                        const localRdv: RendezVous = {
                            ...rdvToSend,
                            id: Date.now(), // ID temporaire
                            patient: fullPatient
                        };
                        this.rendezVous.push(localRdv);
                        this.filterRendezVous();
                        this.updateCalendarFromRendezVous();
                    }
                    this.cancelForm();
    ...
    editRendezVous(rdv: RendezVous): void {
        ...
        // Stocker l'ID numérique du patient pour le select (ngValue préserve le type)
        if (rdv.patient && typeof rdv.patient === 'object' && rdv.patient.id) {
            this.currentRendezVous.patient = Number(rdv.patient.id) as any;
        }
        if (rdv.dateHeure) {
    ...
    getPatientName(patientId: string): string {
        ...
        return patient ? patient.nomComplet : 'Patient inconnu';
    }
