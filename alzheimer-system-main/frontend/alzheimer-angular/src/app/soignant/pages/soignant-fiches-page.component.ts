...
export class SoignantFichesPageComponent implements OnInit {
  ...
  loadData(): void {
    ...
    // Charger les patients depuis le service assistance-quotidienne
    this.http.get<any[]>(`${environment.apiUrl}/users`).subscribe({
      next: (users) => {
        ...
        this.patients = patientsFromUsers as Patient[];
      },
      error: (err) => {
        console.error('Erreur chargement patients:', err);
        // Fallback: essayer le service patient
  ...
  executeSendToDoctor(): void {
    ...
    this.sending = true;
    const ficheId = this.sendTarget.id;
    
    this.ficheApi.marquerEnvoye(ficheId).subscribe({
      next: (updated) => {
        // Mettre à jour la fiche dans la liste principale
        const idx = this.fiches.findIndex(f => f.id === updated.id);
        if (idx >= 0) {
          this.fiches[idx] = { ...updated };
        }
        
        // Mettre à jour dans la liste filtrée
        const filteredIdx = this.filteredFiches.findIndex(f => f.id === updated.id);
        if (filteredIdx >= 0) {
          this.filteredFiches[filteredIdx] = { ...updated };
        }
        
        // Mettre à jour dans la liste paginée
        const pagedIdx = this.pagedFiches.findIndex(f => f.id === updated.id);
        if (pagedIdx >= 0) {
          this.pagedFiches[pagedIdx] = { ...updated };
        }
        
        // Forcer la détection de changement
        this.pagedFiches = [...this.pagedFiches];
