...
import { sanitizeInput } from '../../shared/validation.utils';
...
export class FicheTransmissionHebdoComponent implements OnInit {
  ...
  private createForm(): FormGroup {
    return this.fb.group({
      ...
      commentaireLibre: ['', Validators.maxLength(2000)],
  ...
  onSubmit(): void {
    ...
    if (formValue.commentaireLibre) formValue.commentaireLibre = sanitizeInput(formValue.commentaireLibre);
    if (formValue.alimentation?.details) formValue.alimentation.details = sanitizeInput(formValue.alimentation.details);
    const patientIdNum = this.patientId ? parseInt(this.patientId, 10) : null;

    // Build API payload
    const apiPayload: any = {
      patient: patientIdNum ? { id: patientIdNum } : null,
      dateFiche: this.dateDuJour,
      statut: 'envoye',
      patientInfoJson: JSON.stringify(formValue.patientInfo),
      soignantInfoJson: JSON.stringify(formValue.soignantInfo),
      observanceMedicamentsJson: JSON.stringify(formValue.observanceMedicaments),
      alimentationJson: JSON.stringify(formValue.alimentation),
      vieSocialeJson: JSON.stringify({
        activitesRealisees: formValue.vieSociale?.activitesRealisees || [],
        interaction: formValue.vieSociale?.interaction || 'normale',
        hygiene: formValue.vieSociale?.hygiene || 'autonome',
        sommeil: formValue.vieSociale?.sommeil || 'calme'
      }),
      suiviDirectivesJson: JSON.stringify(formValue.suiviDirectives || []),
      signatureSoignant: formValue.signatureSoignant || false,
      commentaireLibre: formValue.commentaireLibre || ''
    };

    this.ficheApi.create(apiPayload).subscribe({
      next: (saved) => {
        console.log('Fiche de transmission sauvegardée en BDD, id:', saved.id);
  ...
  private marquerTousLesChampsTouched(): void {
    this.markFormGroupTouched(this.ficheForm);
  }
