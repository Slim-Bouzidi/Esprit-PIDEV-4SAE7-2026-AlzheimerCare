    ...
    public MissionResponseDto dispatchMission(MissionDispatchRequestDto dto, LocalDateTime now) {
        ...
        if (assignee != null && StringUtils.hasText(assignee.getEmail())) {
            ...
            if (StringUtils.hasText(adminNotificationEmail)) {
                try {
                    EmailSendOutcome fallbackOutcome =
                            emailNotificationService.sendMissionAssigneeMissingNotice(
                                    adminNotificationEmail.trim(),
                                    saved,
                                    saved.getAssignedMemberId(),
                                    dto.getPatientId(),
                                    alertType.name());
                    logMissionEmailOutcome("MISSION_ASSIGNED_ADMIN_NO_MEMBER_EMAIL", saved.getId(), fallbackOutcome);
                } catch (Exception ex) {
                    log.error(
                            "Mission {} created but admin fallback email (no assignee email) failed: {}",
                            saved.getId(),
                            ex.toString(),
                            ex);
                }
                ...
                log.warn(
                        "[EMAIL] Mission {} created for member {} with no assignee email and support.network.mail.admin-email is unset — no email will be sent.",
                        saved.getId(),
                        saved.getAssignedMemberId());
            }
        }
