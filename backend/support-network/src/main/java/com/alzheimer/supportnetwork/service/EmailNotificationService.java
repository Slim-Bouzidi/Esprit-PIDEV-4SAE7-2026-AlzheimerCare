...
public class EmailNotificationService {
    ...
    /**
     * Mission was created from the UI but the assignee has no email on file, so they cannot receive accept/decline
     * links. Notifies the admin inbox instead (same operational toggle as mission-assigned mail).
     */
    public EmailSendOutcome sendMissionAssigneeMissingNotice(
            String to, Mission mission, long assignedMemberId, long patientId, String alertTypeLabel) {
        if (!sendMissionAssignedEnabled) {
            log.info(
                    "[BREVO SKIP] sendMissionAssigneeMissingNotice reason=TOGGLE_OFF support.network.mail.send-on-mission-assigned=false "
                            + "missionId={}",
                    mission != null ? mission.getId() : null);
            return EmailSendOutcome.skipped(
                    EmailSendStatus.SKIPPED_DISABLED, "send-on-mission-assigned=false");
        ...
        if (mission == null) {
            log.warn("[BREVO SKIP] sendMissionAssigneeMissingNotice reason=NULL_MISSION");
            return EmailSendOutcome.skipped(EmailSendStatus.SKIPPED_NULL_MISSION, "mission is null");
        }
        String subject = "[Support Network] Mission #" + mission.getId() + " — assignee has no email";
        String body =
                "A mission was created (e.g. from \"Créer une mission\" in the patient network UI).\n"
                        + "Mission ID: "
                        + mission.getId()
                        + "\n"
                        + "Assignee support member ID: "
                        + assignedMemberId
                        + " — this member has no email in the database, so no transactional email could be sent to them.\n"
                        + "Add an email address on the support member record to enable accept/decline links by mail.\n"
                        + "Patient ID: "
                        + patientId
                        + "\n"
                        + "Alert type: "
                        + (alertTypeLabel != null ? alertTypeLabel : "")
                        + "\n"
                        + "Title: "
        ...
        String normalized = normalizeEmailAddress(to);
        log.info(
                "[BREVO FLOW] sendMissionAssigneeMissingNotice missionId={} memberId={} intendedRecipient={} subject=\"{}\"",
                mission.getId(),
                assignedMemberId,
                maskEmail(normalized),
                subject);
        ...
        if (!isValidEmail(normalized)) {
            log.warn(
                    "[BREVO SKIP] sendMissionAssigneeMissingNotice reason=INVALID_RECIPIENT_FORMAT missionId={} raw={}",
                    mission.getId(),
                    maskEmail(normalized));
            return EmailSendOutcome.skipped(
                    EmailSendStatus.SKIPPED_INVALID_RECIPIENT, "invalid recipient: " + maskEmail(normalized));
        }
        return sendOrBrevo(normalized, subject, body, null, "mission-assignee-missing");
    }
