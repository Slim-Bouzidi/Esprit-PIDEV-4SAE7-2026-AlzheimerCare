...
public class SupportMemberService {
    private static final Logger log = LoggerFactory.getLogger(SupportMemberService.class);
    ...
    private void applyBasics(SupportMemberDto dto, SupportMember m) {
        m.setFullName(dto.getFullName());
        m.setPhone(dto.getPhone());
        if (dto.getEmail() != null) {
            String trimmed = dto.getEmail().trim();
            m.setEmail(trimmed.isEmpty() ? null : canonicalizeEmailFromUserService(trimmed));
        } else {
            m.setEmail(null);
        }
    ...
    /**
     * support-network -> user-service (OpenFeign):
     * if the email belongs to an existing platform user, store canonical email from user-service.
     * Fallback: keep user-entered email to avoid breaking existing flows.
     */
    private String canonicalizeEmailFromUserService(String email) {
        try {
            List<UserServiceUserDto> users = userServiceClient.getAllUsers();
            if (users != null) {
                for (UserServiceUserDto user : users) {
            ...
            log.warn("OpenFeign lookup user-service failed while canonicalizing member email: {}", ex.getMessage());
        }
        return email;
    }
