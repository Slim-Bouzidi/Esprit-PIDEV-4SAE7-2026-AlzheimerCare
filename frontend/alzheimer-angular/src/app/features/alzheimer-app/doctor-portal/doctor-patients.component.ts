...
export class DoctorPatientsComponent implements OnInit {
    ...
    soignantApiAvailable: boolean = true;
    ...
    loadSoignants(): void {
        this.userService.getByRole('CAREGIVER').pipe(
            catchError(() => this.userService.getByRole('SOIGNANT')),
            switchMap((caregivers) => {
                if (caregivers.length > 0) {
                    return of(caregivers);
                }
                return this.userService.getByRole('SOIGNANT').pipe(
                    catchError(() => of([]))
                );
            })
        ).subscribe({
            next: (soignants) => {
                ...
                this.soignantApiAvailable = true;
            },
            error: () => {
                this.soignantApiAvailable = false;
                this.soignants = [];
            }
        });
    }
    ...
    getSoignantLabel(soignant: User): string {
        const fullName = `${soignant.firstName ?? ''} ${soignant.lastName ?? ''}`.trim();
        if (soignant.nom && soignant.nom.trim().length > 0) {
            return soignant.nom;
        }
        if (fullName.length > 0) {
            return fullName;
        }
        if (soignant.email && soignant.email.trim().length > 0) {
            return soignant.email;
        }
        return `Caregiver #${soignant.id ?? '?'}`;
    }
