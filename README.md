...
Git Push
  └─► GitHub Webhook
        └─► Jenkins (192.168.80.128:8080)
              ├─► SonarQube Analysis (192.168.80.128:30005)
              ├─► Maven / npm Build
              ├─► Docker Build + Push → Nexus (192.168.80.128:30082)
...
## Service Access (Laptop 2 — 192.168.80.128)

| Service        | URL                                   | Credentials        |
|----------------|---------------------------------------|--------------------|
| Frontend       | http://192.168.80.128:30420           | Keycloak login     |
| API Gateway    | http://192.168.80.128:30080           | All API calls      |
| Keycloak       | http://192.168.80.128:30081           | admin / admin      |
| Eureka         | http://192.168.80.128:30761           | Service registry   |
| Jenkins        | http://192.168.80.128:8080            | CI/CD              |
| Nexus UI       | http://192.168.80.128:30000           | admin / admin1234  |
| Nexus Docker   | 192.168.80.128:30082                  | Docker registry    |
| SonarQube      | http://192.168.80.128:30005           | admin / admin      |
| Prometheus     | http://192.168.80.128:30090           | Metrics            |
| Grafana        | http://192.168.80.128:30030           | admin / admin      |
| PhpMyAdmin     | http://192.168.80.128:30085           | DB admin           |
...
kubectl create secret docker-registry nexus-creds \
  --docker-server=192.168.80.128:30082 \
...
```json
{
  "insecure-registries": ["192.168.80.128:30082"]
}
```
...
```bash
sudo mkdir -p /etc/containerd/certs.d/192.168.80.128:30082
cat <<EOF | sudo tee /etc/containerd/certs.d/192.168.80.128:30082/hosts.toml
[host."http://192.168.80.128:30082"]
...
1. Open http://192.168.80.128:30000 and log in (admin / admin1234)
...
```bash
docker login 192.168.80.128:30082 -u admin -p admin1234
docker pull hello-world
docker tag hello-world 192.168.80.128:30082/hello-world:test
docker push 192.168.80.128:30082/hello-world:test
```
...
1. Open http://192.168.80.128:30030 → admin / admin
...
1. Open http://192.168.80.128:30005 → admin / admin (change password on first login)
...
- Name: `SonarQube`
- Server URL: `http://192.168.80.128:30005`
...
- Jenkins URL: `http://192.168.80.128:8080`
...
2. Payload URL: `http://192.168.80.128:8080/github-webhook/`
...
curl http://192.168.80.128:30761/eureka/apps | grep -o '<app>.*</app>'
```
...
- Eureka (http://192.168.80.128:30761): all 7 microservices registered
...
- Frontend accessible: http://192.168.80.128:30420
...
- Keycloak client `alzheimer-angular-client` has `redirectUris` set to `http://192.168.80.128:30420/*`
